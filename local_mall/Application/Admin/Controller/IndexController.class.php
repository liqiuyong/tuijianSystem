...
class IndexController extends AdminController
{
    ...
    public function is_realCheck(){
        if (session('user_id')){
            ...
            if ($user){
                if ($user['is_checked'] == 2){
                    $this->redirect("http://__ROOT__/geren");
                    exit();
                }
            }
        }
...
//            $this->assign('userlist', $userlist);
    ...
    public function tuijian()
    {
        ...
        if (session('user_id')) {
            ...
            $Page = new Page($count,8);// 实例化分页类 传入总记录数和每页显示的记录数(10)
            $Page->lastSuffix=false;
            $Page->setConfig('header','<div style="margin-left: 20px">共<b>%TOTAL_ROW%</b>条记录&nbsp;&nbsp;第<b>%NOW_PAGE%</b>页/共<b>%TOTAL_PAGE%</b>页</div>');
            $Page->setConfig('first','<img src="/Public/Admin/images/new/page-pre1.png">');
            $Page->setConfig('prev','<img src="/Public/Admin/images/new/page-pre2.png">');
            $Page->setConfig('next','<img src="/Public/Admin/images/new/page-pre2.png">');
            $Page->setConfig('last','<img src="/Public/Admin/images/new/page-pre1.png">');
            $Page->setConfig('theme','%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END% %HEADER%');
            $Page->rollPage=5;// 分页栏每页显示的页数

            $show       = $Page->show();// 分页显示输出
            //分页显示改成链接带 get参数的形式
            $show_new = str_replace("Admin/Index/tuijian/m/admin/p/","tuijian.html?p=",$show);
...
//            $userlist_2_totle = $userlist_2_checked_count + $userlist_2_unchecked_count;
            ...
            $show       = $Page->show();// 分页显示输出
            //分页显示改成链接带 get参数的形式
            $show_new = str_replace("Admin/Index/tuijian_2/m/admin/p/","tuijian_2.html?p=",$show);
    ...
    public function geren()
    {
        if ($_POST) {
            ...
            $tr_password = isset($_POST['tr_password']) ? trim($_POST['tr_password']) : '';
            $sub_bank_name = isset($_POST['sub_bank_name']) ? trim($_POST['sub_bank_name']) : '';
...
//            $res = $user_model->where('real_card =' . $real_card)->find();
            ...
            if ( empty($user['tr_password']) ){
                if (empty($tr_password)) {
                    $this->ajax_msg('请设置交易密码');
                }
                if (strlen($tr_password) > 20 || strlen($tr_password) < 6) {
                    $this->ajax_msg("密码必须为6-20位的字符串", 0);

                }

                if (preg_match("/^\d*$/", $tr_password)) {
                    $this->ajax_msg("密码必须包含字母", 0);//全数字
                }

                if (preg_match("/^[a-z]*$/i", $tr_password)) {
                    $this->ajax_msg("密码必须包含数字", 0);//全字母
                }


                if (!preg_match("/^[a-z\d]*$/i", $tr_password)) {
                    $this->ajax_msg("密码只能包含数字和字母", 0);//有数字有字母 ";
                }

                $tr_password = md5(md5($tr_password));
            }else{
                $tr_password = $user['tr_password'];
            }
...
//                dump("输入字符长度过长");
            ...
            $data_save = $user_model->where(array('user_id' => $_SESSION['user_id']))->data(array('tr_password'=>$tr_password ,'real_name' => $real_name,'real_card'=>$real_card, 'bank_card' => $bank_card, 'bank_name' => $bank_name,'sub_bank_name'=>$sub_bank_name , 'is_checked' => 1))->save();
...
//    订单列表页面
    public function order_list(){
        ...
        if (session('user_id')){
            ...
            $show       = $Page->show();// 分页显示输出
            //分页显示改成链接带 get参数的形式
            $show_new = str_replace("Admin/Index/order_list/m/admin/p/","order_list.html?p=",$show);
...
//            }
    ...
    public function charge_status(){
        ...
        if ($_POST){
            ...
            $data['charge_type'] = isset($_POST['charge_type']) ? trim($_POST['charge_type']) : '';
            ...
            if ($order){
                if ($order['charge_status'] == 1 ){
                    $res = $order_model->where('order_num = '.$data['order_num'])->data(['charge_status'=>2, 'charge_type'=>$data['charge_type'], 'complete_time'=>time()])->save();
...
//            $real_card = $_POST['real_card'];
            $real_name = $_POST['real_name'];
            $bank_name = $_POST['bank_name'];
            $sub_bank_name = $_POST['sub_bank_name'];
            $tr_password = $_POST['tr_password'];



//            验证提现金额
            if (($charge_money%100) != 0){
                $this->ajax_msg('提现金额必须为100的整数倍!');
            }
...
//              发送验证码
     ...
     * 身份证图片上传
     */
    public function upload_img_old(){
...
//存储过程  1.判断文件是否有错--->2.没错的情况下,修改文件名,文件另存为   3. 返回json给前端
        ...
        for ($i=0;$i<$fileNum;$i++){
            ...
            $filepath = "./Uploads/".session('user_id')."_".$name.".".$cur_ext;
            unlink($filepath);
            if (move_uploaded_file($fileArr['tmp_name'][$i],$filepath)){
                //存储上传文件的路径
                $imgModel = M('card_img');

                $res = $imgModel->where(['user_id'=>session('user_id'),'upload_src'=>$filepath])->find();

                if ($res){
                    $res = $imgModel->where("upload_src=".$filepath)->save(['user_id'=>session('user_id'),'upload_src'=>$filepath,'addtime'=>time()]);

                }else{
                    $res = $imgModel->data(['user_id'=>session('user_id'),'upload_src'=>$filepath,'addtime'=>time()])->add();
                }

//                $res = $imgModel->data(['user_id'=>session('user_id'),'upload_src'=>$filepath,'addtime'=>time()])->add();
//                $res = $imgModel->where("upload_src=".$filepath)->find();
//                if ($res){
//                    $res = $imgModel->where("upload_src=".$filepath)->save(['user_id'=>session('user_id'),'upload_src'=>$filepath,'addtime'=>time()]);
//
//                }else{
